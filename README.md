...
```bash
omarchy theme install /home/seb/Projects/omarchy-theme-darkula
```

That clones the theme into `~/.config/omarchy/themes/darkula` and applies it right away. 
...
`colors.toml` is the source of truth. Omarchy generates terminal, VSCode, Chromium, foot, ghostty, kitty and shell colours from it. `btop.theme`, `icons.theme` and `libadwaita-gtk.css` are checked in so those apps keep the same palette without waiting on templates.

`libadwaita-gtk.css` supplies `@define-color` overrides for GTK4/libadwaita apps such as Nautilus, file dialogs and Geary. Stock Omarchy does not deploy GTK4 palette CSS; this file is for a patched install that generates it from `colors.toml` and writes `~/.config/gtk-4.0/gtk.css` on theme switch.
